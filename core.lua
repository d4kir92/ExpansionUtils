...
local function GetVaultStatus(vaultData, name)
	...
	for i, data in pairs(vaultData[name]) do
		...
		if data.progress > data.threshold then status = color .. data.threshold .. "|cFFFFFFFF/" .. color .. data.threshold end
...
local function GetVaultStatusIlvl(vaultData, name)
	...
	for i, data in pairs(vaultData[name]) do
		...
		if itemLink then ilvl = GetDetailedItemLevelInfo(itemLink) end
		local color = "|cFFFFFFFF"
		if data.progress == 0 then
			color = "|cFFFF0000"
		elseif data.progress >= data.threshold then
...
function ExpansionUtils:FixCastBar(castbar, notInterruptible)
	...
	if notInterruptible then
		...
		if castbar.BarBorder then castbar.BarBorder:Show() end
		castbar:SetStatusBarColor(0.5, 0.5, 0.5)
	else
		castbar.BorderShield:Hide()
		if castbar.BarBorder then castbar.BarBorder:Hide() end
	end
end
...
ExpansionUtils:OnEvent(fEV, function()
	...
	ExpansionUtils:SetVersion(133740, "1.2.36")
	EVTAB = EVTAB or {}
	...
	if ExpansionUtils:GetWoWBuild() == "RETAIL" then
		...
		if ExpansionUtils:GV(EVTAB["MMBtnReshiWrap"], "MMBTNRESHIIWRAP", true) then
			local btnReshii = ExpansionUtils:CreateMinimapButton({
				...
				["funcL"] = function()
					...
					if GenericTraitFrame then
						if GenericTraitFrame.SetSystemID then GenericTraitFrame:SetSystemID(29) end
						if GenericTraitFrame.SetTreeID then GenericTraitFrame:SetTreeID(1115) end
			...
			btnReshii:SetScript("OnUpdate", function()
	...
	if ExpansionUtils:GetWoWBuild() ~= "RETAIL" then
		...
		local function UpdateShieldIcon(unit)
			...
			if notInterruptible == nil then _, _, _, _, _, _, notInterruptible = UnitChannelInfo(unit) end
			if unit == "target" then
...
ExpansionUtils:OnEvent(fCUI, function(sel, event, ...)
	...
	elseif event == "READY_CHECK_FINISHED" then
		if ChallengesKeystoneFrame then
			...
			C_Timer.After(6, function() ChallengesKeystoneFrame.readyCheckText:SetText("") end)
		end
	else
		...
		if addonName == "Blizzard_ChallengesUI" then
			...
			if ChallengesKeystoneFrame then
				...
				if startButton then
					...
					ChallengesKeystoneFrame.readyCheck:SetScript("OnClick", function()
						lastCount = 1
						DoReadyCheck()
						if READY then
							ChallengesKeystoneFrame.readyCheckText:SetText("|cffffff00" .. lastCount .. "/" .. GetNumGroupMembers() .. " " .. READY)
					...
					for x, w in pairs(countdowns) do
						...
						ChallengesKeystoneFrame["countdown" .. w]:SetScript("OnClick", function()
